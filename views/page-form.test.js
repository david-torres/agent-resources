...
// Harness guide: use FORM() below for plain values with no embedded quotes
// (it's a raw template literal, not HTML-escaped); use renderPageForm()
// above for anything that needs real escaping, e.g. adversarial titles.
//
...
// Regression: the deleted script's title handler re-armed auto-fill
// whenever the slug was empty, regardless of the manual-edit flag
// (`!slugInput.value || slugInput.dataset.autoGenerated === 'true'`). The
// help text under the slug field still promises "Leave empty to
// auto-generate from title" -- clearing a hand-edited slug must resume
// following, and it must keep following on subsequent keystrokes (not
// just refill once and go dead again).
test('clearing a hand-edited slug re-arms auto-fill, which then keeps following', async () => {
  await render(FORM('', ''));
  const title = document.getElementById('title');
  const slug = document.getElementById('slug');

  // 1. Type a title -> slug follows.
  title.value = 'The Silent Harbor';
  title.dispatchEvent(new window.Event('input', { bubbles: true }));
  await tick();
  expect(slug.value).toBe('the-silent-harbor');

  // 2. Hand-edit the slug -> stays put while the title keeps changing.
  slug.value = 'custom-slug';
  slug.dispatchEvent(new window.Event('input', { bubbles: true }));
  await tick();
  title.value = 'The Silent Harbor Again';
  title.dispatchEvent(new window.Event('input', { bubbles: true }));
  await tick();
  ...
  // 4. Type more into the title -> slug auto-fills again.
  title.value = 'Reborn Harbor';
  title.dispatchEvent(new window.Event('input', { bubbles: true }));
  await tick();
  expect(slug.value).toBe('reborn-harbor');

  // 5. Keep typing the title -> slug keeps following, proving `auto` was
  // genuinely re-armed rather than a one-shot refill.
  title.value = 'Reborn Harbor Rises';
  title.dispatchEvent(new window.Event('input', { bubbles: true }));
  await tick();
  expect(slug.value).toBe('reborn-harbor-rises');
});
...
// --- is_news checkbox ---
//
// Unlike is_published (which defaults checked for a new page), is_news must
// default UNCHECKED: most pages are not news, and a brand-new page has no
// `page` in context at all.
test('the is_news checkbox is unchecked by default for a new page', async () => {
  await renderPageForm({ page: null });
  const checkbox = document.getElementById('is_news');
  expect(checkbox).not.toBeNull();
  expect(checkbox.checked).toBe(false);
});

test('the is_news checkbox is checked when editing a page with is_news: true', async () => {
  await renderPageForm({ page: { id: 11, title: 'Patch Notes', slug: 'patch-notes', is_news: true } });
  const checkbox = document.getElementById('is_news');
  expect(checkbox.checked).toBe(true);
});

test('the is_news checkbox is unchecked when editing a page with is_news: false', async () => {
  await renderPageForm({ page: { id: 12, title: 'About', slug: 'about', is_news: false } });
  const checkbox = document.getElementById('is_news');
  expect(checkbox.checked).toBe(false);
});
