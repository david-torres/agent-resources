    ...
    if (character.is_public === false && (!profile || character.creator_id !== profile.id)) {
      ...
      try { 
        if (characterClass && profile) {
          const { data: unlocked } = await isClassUnlocked(profile.user_id, characterClass.id);
          isUnlocked = !!unlocked;
          if (isUnlocked) {
            // Check individual abilities for unlock status
            if (Array.isArray(character.abilities)) {
              for (const ability of character.abilities) {
                if (ability.class_id) {
                  ...
                  if (abilityUnlocked) {
                    abilityDescMap[ability.name] = ability.description || '';
                  } else {
                    abilityDescMap[ability.name] = '';
                  }
                }
              }
            }
            
            // Check individual gear items for unlock status
            if (Array.isArray(character.gear)) {
              for (const gear of character.gear) {
                if (gear.class_id) {
                  ...
                  if (gearUnlocked) {
                    gearDescMap[gear.name] = gear.description || '';
                  } else {
                    gearDescMap[gear.name] = '';
                  }
                }
              }
            }
