...
```js
  ...
  Alpine.data('pageSlug', (initialTitle, initialSlug) => ({
    ...
    // Two conditions, both from the deleted script's title handler:
    //   if (!slugInput.value || dataset.autoGenerated === 'true') { … }
    // An EMPTY slug re-arms auto-fill regardless of the manual flag — that
    // is what makes the form's own help text ("Leave empty to auto-generate
    // from title") true. Checking only `auto` leaves a cleared slug dead
    // forever once the user has touched it. Re-setting `auto = true` matters
    // too: a refilled slug is auto-generated again and must keep following.
    onTitle() {
