...
**`FORBIDDEN` moves.** `test/load-prerelease-classes.test.js:28` forbids `rules_edition`, `rules_version` and `base_class_id` in every payload. For **fork plans specifically** `rules_edition` and `base_class_id` are now required. `rules_version` is **not**: the insert supplies it from `NEW_ROW_RULES_VERSION` for creates and forks alike, which is exactly what keeps it out of every update payload, so it stays forbidden in all of them. Split the assertion: non-fork payloads keep the full `FORBIDDEN` list; fork payloads assert `id`, `base_class_id`, `rules_edition` and `content_format` are present and correct, and that `rules_version`, `is_public`, `status`, `teaser`, `image_url` and `image_crop` are still absent.

**A book with no remap file skips the remap machinery entirely.** Task 1 already made the read conditional (`:281` reads the file only when `book.remap` is set, otherwise `[]`) and the orphan scan at `:313-314` already runs either way — verify and pin that, rather than rewriting it. The requirement is that with `book.remap === null` the remap is `[]` and the read is skipped, but the orphan scan **still runs**. Forking orphans nothing by construction, so the scan should come back empty — and that is the point. A scan that is skipped cannot tell you the projection is wrong; a scan that runs and reports zero can.

**`rowByName` goes ambiguous.** `:385-388` maps payload name to row for the publish step. Publishing must key on the **plan**, not the name — carry the created row back onto its plan after the insert and publish from `plan.row.id`. The name-keyed map itself cannot actually hold two `Berserker` entries, because there is one plan per record; the genuine name-keyed defect is next door in `projectImport`, which maps `publishedByLoad` over every catalogue row by name and so projects a fork's **parent** as published by this load. Fix that one too.
