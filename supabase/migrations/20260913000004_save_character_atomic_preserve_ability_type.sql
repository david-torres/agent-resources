-- Restates the whole function to stop an absent ability type from being read as
-- 'core'. The edit form submits abilities as bare "Class::Ability" strings with
-- no type, so COALESCE(..., 'core') in the desired CTE silently retagged every
-- advanced ability on an ordinary save. An absent type now means "leave the
-- stored tag alone"; only a genuinely new row falls back to 'core'.
--
-- Every block except the p_abilities one is byte-identical to
-- 20260913000003_save_character_atomic_pseudo_class.sql.
