-- Restates the whole function to carry characters.pseudo_class_tagline and
-- .pseudo_class_description (20260913000002) through the atomic path. Every
-- block except the INSERT and UPDATE column lists is byte-identical to
-- 20260913000001_save_character_atomic_ability_type.sql.
--
-- jsonb_populate_record alone is not enough here: the INSERT and UPDATE name
-- their columns explicitly, so a column the list does not mention is dropped
-- silently -- which is what happened to both of these on every aspiring save.
