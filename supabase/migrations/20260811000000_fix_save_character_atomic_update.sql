-- Repair save_character_atomic's UPDATE branch.
--
-- 20260710000000_atomic_character_writes.sql:62 wrote
--
--   UPDATE public.characters AS current
--   SET ...
--   FROM jsonb_populate_record(current, p_character) AS record
--
-- which references the UPDATE's own target alias inside its FROM clause.
-- Postgres rejects that at execution time:
--
--   ERROR: invalid reference to FROM-clause entry for table "current"
--
-- so every character EDIT through the atomic path failed, and PUT /characters
-- answered 500. Creates were unaffected -- they take the INSERT branch, which
-- is why models/character-atomic.integration.test.js (create and create-
-- rollback only) stayed green and the defect reached CI unnoticed.
--
-- The fix loads the existing row into `saved` first, then populates the record
-- from `saved` rather than from the target alias. The pre-read doubles as the
-- not-found check, so a character the caller does not own still raises rather
-- than silently updating nothing.
--
-- Forward-only: this replaces the function rather than editing the original
-- migration, so databases that already applied it converge too. CREATE OR
-- REPLACE with an unchanged signature preserves the REVOKE/GRANT set by
-- 20260710000001 and 20260710000002.
