-- Restates the whole function so the INSERT and UPDATE paths carry
-- characters.aspiring_signatures, added by
-- 20260920000000_characters_aspiring_signatures.sql (ENCLAVE: Aspirant V1,
-- pg. 90: an Aspiring character's three Signature picks are treated as
-- belonging to its own Class).
--
-- The UPDATE branch needs no COALESCE: its source is
-- jsonb_populate_record(saved, p_character), whose base is the stored row,
-- so a key absent from p_character already keeps the stored value. That is
-- the same mechanism that preserves pseudo_class_tagline across an edit that
-- never mentions it, and it is what lets an edit-form save -- which does not
-- submit this field -- leave the pool untouched.
--
-- Every block except the three touching aspiring_signatures is byte-identical
-- to 20260919000002_save_character_atomic_trait_stat.sql.
...
BEGIN
  IF p_character_id IS NULL THEN
    INSERT INTO public.characters (
      ...
      pseudo_class_tagline, pseudo_class_description, aspiring_signatures
    )
    SELECT
      ...
      record.pseudo_class_tagline, record.pseudo_class_description, COALESCE(record.aspiring_signatures, '[]'::jsonb)
